...
def get_model(cp_path="transferin/model.pt"):
    p_dim = 2560
    n_layers = 1
    config = ReactEmbedConfig(p_dim=p_dim, m_dim=768, n_layers=1, hidden_dim=256, dropout=0.3)
    model = ReactEmbedModel(config).eval()
    task_model = LinFuseModel(DataType.PROTEIN, fuse_model=model, n_layers=n_layers)
